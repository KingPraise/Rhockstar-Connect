...
| **High** | **25** | 38.5% | Broken DM routing, fake ATS statuses, unrestricted employer access to competitor candidates, self-locking profile updates, silent notifications, missing runtime indexes, Next.js image crashes, plaintext password bypass. |
...
| **Low** | **5** | 7.7% | Character encoding glitches (`?` instead of `₦`), Netlify fallback URLs, duplicate return statements, dead video refs, hardcoded mock AI assistant responses. |
...
| **ROUT** | Frontend Routing, Auth Guards & Hydration | 9 | 1 | 4 | 4 | 0 |
| **DEAD** | Interactive Controls & Dead UI Elements | 10 | 0 | 5 | 4 | 1 |
...
| **TOTALS** | **All Categories** | **65** | **13** | **25** | **22** | **5** |
...
#### ROUT-09: Clickable Parent Div Wrapping Interactive Button in Premium Card
- **Severity**: **Medium**
- **Component**: Premium Subscription Page
- **File & Lines**: `src/app/(dashboard)/premium/page.tsx:193-202`
- **User Flow / Scenario**: A user clicks on a subscription card near the "Upgrade Now" button.
...
- **Evidence Snippet**:
  ...
  ```
- **Recommended Fix**:
  Remove `onClick` from the outer card wrapper and keep click handling confined to `<PaymentButton>`.
