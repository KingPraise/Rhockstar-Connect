              ...
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group/item is-active">
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] neo-card p-6 bg-slate-800/50 group-hover/item:border-brand-purple/50 transition-colors shadow-lg">
                  <div className="flex flex-col">
                    ...
                    <p className="text-slate-300 text-sm mt-2 font-medium bg-white/5 w-fit px-3 py-1 rounded-full border border-white/10">Jan 2024 - Present</p>
            ...
            {profile?.education ? (
              <div className="flex gap-5 group/edu p-4 -mx-4 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer">
                ...
                <div className="flex flex-col justify-center flex-grow">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold text-xl text-white group-hover/edu:text-emerald-500 transition-colors">{profile.education}</h3>
        ...
        <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-wrap gap-2">
              {profile?.skills && profile.skills.length > 0 ? (
                profile.skills.map((skill: string, index: number) => (
                  <span key={index} className="px-4 py-2 bg-slate-800/80 hover:bg-blue-500/20 text-slate-300 hover:text-blue-400 font-medium rounded-xl border border-white/5 hover:border-blue-500/30 transition-all cursor-default shadow-sm hover:shadow-[0_0_10px_rgba(59,130,246,0.2)]">
                    {skill}
                  </span>
                ))
              ) : (
          ...
          <div className="neo-card flex flex-col gap-6 relative overflow-hidden group bg-slate-900/40 backdrop-blur-md border-white/5 shadow-2xl">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center p-3 rounded-xl hover:bg-white/5 transition-colors cursor-default border border-transparent hover:border-white/5">
                <span className="font-bold text-white">English</span>
                <span className="text-xs font-semibold px-2.5 py-1 bg-amber-500/10 text-amber-400 rounded-full border border-amber-500/20">Native</span>
              </div>
            </div>
          </div>
